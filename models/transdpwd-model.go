...
func Fetch_transdpwdHome(idmasteragen string) (helpers.Response, error) {
	...
	for row.Next() {
		...
		obj.Transdpwd_nmmember = _GetInfoMember(idmasteragen, idagenmember_db, tbl_mst_member)
...
func _GetInfoBank(idmasteragen, idagenmember, tipe, table_memberbank string, idrecord int) string {
	con := db.CreateCon()
	ctx := context.Background()
	info := ""
	bank_id := ""
...
func _GetInfoMember(idmasteragen, idagenmember, table string) string {
	con := db.CreateCon()
	ctx := context.Background()
	username_member_db := ""
	name_member_db := ""
